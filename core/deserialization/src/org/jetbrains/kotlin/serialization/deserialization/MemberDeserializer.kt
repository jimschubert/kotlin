...
class MemberDeserializer(private val c: DeserializationContext) {
    fun loadProperty(proto: ProtoBuf.Property): PropertyDescriptor {
        val flags = if (proto.hasFlags()) proto.flags else loadOldFlags(proto.oldFlags, Flags.HAS_CONSTANT)
    ...
    private fun loadOldFlags(flags: Int, lastFlag: Flags.FlagField<*>): Int {
        @Suppress("DEPRECATION")
        val mask = (1 shl (lastFlag.offset + 1)) - 1
        val lowSixBits = flags and 0x3f
        val rest = ((flags shr 8) and mask) shl 6
        return lowSixBits + rest
    }

    fun loadFunction(proto: ProtoBuf.Function): FunctionDescriptor {
        val flags = if (proto.hasFlags()) proto.flags else loadOldFlags(proto.oldFlags, Flags.IS_EXTERNAL_FUNCTION)
